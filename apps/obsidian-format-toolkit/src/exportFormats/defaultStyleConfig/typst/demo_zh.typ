...
本文件用于展示 Doc Weaver 的 Typst 导出效果。插件会将 Obsidian 中的 Markdown 笔记、嵌入笔记与附件（如图片与绘图）整理为结构化的 Typst 文档，便于进一步排版并生成 PDF。你可以把它当作默认主题的示例。
...
基础下划线：#DW_underline[这是下划线文本]

挖空效果：#DW_underline(show_content: false)[隐藏的答案]

多行填空（自动计算行数）：#DW_underline(line_break: true, show_content: false)[这是一段隐藏内容，系统会根据文本长度自动计算需要的行数，为每行添加下划线。这是一段隐藏内容，系统会根据文本长度自动计算需要的行数，为每行添加下划线]
